...
| The init sequence is deliberately the *union* of two drivers | LilyGO's factory driver sends only DISPOFF/SLPIN/SLPOUT/DISPON and trusts the controller's OTP; Arduino_GFX's `Arduino_AXS15231` also sets NORON, INVOFF, COLMOD=16bpp and the brightness-control block. `panel.cpp` sends both sets - every one is a standard DCS command, and it removes "what did the OTP leave unset" from the list of things that can be wrong. The vendor's 30-entry manufacturer-register table is intentionally **not** used. |
